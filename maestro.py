                                ...
                                if i == RecuperoInfo[j][0]:
                                        if len(RecuperoInfo[j]) > 2:
                                                for k in range(0,len(RecuperoInfo[j][2])):
                                                        if int(message.split("|")[i],16) == RecuperoInfo[j][$
                                                                MQTT_MAESTRO[RecuperoInfo[j][1]] = RecuperoI$
                                                                break
                                                        else:
                                                                MQTT_MAESTRO[RecuperoInfo[j][1]] = ('Code in$
                                        else:
                                                if i == 6 or i == 26 or i == 28:
                                                        MQTT_MAESTRO[RecuperoInfo[j][1]] = float(int(message$

                                                elif i >= 37 and i <=42:
                                                        MQTT_MAESTRO[RecuperoInfo[j][1]] = secTOdhms(int(mes$
                                                else:
                                                        MQTT_MAESTRO[RecuperoInfo[j][1]] = int(message.split$
...
if __name__ == "__main__":
        while True:
                logger.info("Etablissement d'une nouvelle connection au serveur websocket (IP:"+_MCZip+" POR$
